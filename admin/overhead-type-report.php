<?php include 'header.php'; ?>
<?php include 'sidebar.php'; ?>
<?php
if (isset($_POST['search'])) {
    $sm = $_POST['sales'];
    $from = date('Y-m-d', strtotime($_POST['from']));
    $to = date('Y-m-d', strtotime($_POST['to']));
    $getSalesReport = $report->getAllOverheadByDate($sm,$from, $to);
} else {
    $getSalesReport = $report->getAllOverheadReport();
}
?>
<div class="page-wrapper">
    <div class="page-breadcrumb">
        <div class="row">
            <div class="col-12 d-flex no-block align-items-center">
                <h4 class="page-title">Overhead Report</h4>
                <div class="ml-auto text-right">
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb">
                            <li class="breadcrumb-item"><a href="dashboard.php">Home</a></li>
                            <li class="breadcrumb-item active" aria-current="page">overhead report</li>
                        </ol>
                    </nav>
                </div>
            </div>
        </div>
    </div>

    <div class="container-fluid">
        <div class="row">
            <div class="col-md-12">
                <div class="card">
                    <div class="card-body">
                        <h5 class="card-title">All Overhead Report</h5>
                        <div class="box-tools pull-right" align="center">
                            <form class="form-horizontal" method = "post" action="">
                                <div class="row">
                                    <div class="col-md-4">
                                        <div class="form-group">
                                            <label for="lname">Overhead<span style="color: red"> *</span></label>
                                            <select class="form-control select2" name="sales" required>
                                                <option>Select Type</option>
                                                <?php
                                                $getSM = $report->getAllActiveOverhead();
                                                if ($getSM) {
                                                    while ($result = $getSM->fetch_assoc()) {
                                                        ?>
                                                        <option value="<?php echo $result['id']; ?>"><?php echo $result['overhead_name']; ?></option>
                                                        <?php
                                                    }
                                                }
                                                ?>
                                            </select>
                                        </div>
                                    </div>
                                    <div class="col-md-3">
                                        <div class="form-group">
                                            <label for="datepicker-autoclose">From</label>
                                            <input type="text" class="form-control" id="datepicker-autoclose" name="from" autocomplete="off" placeholder="mm/dd/yyyy">
                                        </div>
                                    </div>
                                    <div class="col-md-3">
                                        <div class="form-group">
                                            <label for="datepicker-autoclose">To</label>
                                            <input type="text" class="form-control" id="datepicker-autoclose1" name="to" autocomplete="off" placeholder="mm/dd/yyyy">
                                        </div>
                                    </div>
                                    <div class="col-md-2">
                                        <div class="form-group" style="margin-top: 25px;">
                                            <button type="submit" name ="search" style="width: 120px;" class="btn btn-success">Search</button>
                                        </div>
                                    </div>
                                </div>
                            </form>
                        </div><br>
                        <div class="table-responsive">
                            <table id="buttonTab" class="table table-striped table-bordered display tblReport">
                                <thead>
                                    <tr>
                                        <th style="width:10px;">SL</th>
                                        <th style="width:20px;">Date</th>
                                        <th style="width:20px;">Overhead</th>
                                        <th style="width:20px;">Amount</th>
                                        <th style="width:20px;">Purpose</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <?php
                                    if ($getSalesReport) {
                                        $i = 0;
                                        $Tamnt = 0;;
                                        while ($result = $getSalesReport->fetch_assoc()) {
                                            $i++;
                                            ?>
                                            <tr>
                                                <td><?php echo $i; ?></td>
                                                
                                                <td><?php echo $fm->formatDate($result['overhead_info_date']); ?></td>
                                                <td>
                                                    <?php
                                                    $oh = $medicine->getOneCol('overhead_name', 'overhead', 'id', $result['overhead_info_head']);
                                                    echo $oh;
                                                    ?>
                                                </td>
                                                <td><?php echo $ta = $result['overhead_info_amount']; ?></td>
                                                <td><?php echo $result['purpose']; ?></td>
                                            </tr>
                                            <?php
                                            $Tamnt = $Tamnt + $ta;
                                        }
                                    }
                                    ?>
                                </tbody>
                                <tfoot>
                                <th></th>
                                <th colspan="2">Total</th>
                                <th>
                                    <?php
                                    if (isset($Tamnt)) {
                                        echo $Tamnt;
                                    }
                                    ?>
                                </th>
                                <th></th>
                                </tfoot>
                            </table>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>
<?php include 'footer.php'; ?>







