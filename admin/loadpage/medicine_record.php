<?php
	...
	/*$sql = "SELECT medicine.*,medicine.id as mid,rack.*,generic.*,company.* 
			...
			WHERE medicine.deletion_status = 0
			";*/
			
			$sql = "SELECT medicine.*,medicine.id as mid,generic.*,company.* 
			FROM medicine 
	...
	while( $rows = mysqli_fetch_assoc($resultset) ) {
		...
		$data[] = array(
			...
			'medicine_name' => $rows['medicine_name'].' '.$rows['medicine_strength'].''.$rows['medicine_form'],
			'medicine_generic' => $rows['generic_name'],
			'medicine_from' => $rows['medicine_form'],
			'medicine_strength' => $rows['medicine_strength'],
			'company_name' => $rows['company_name'],
			'medicine_indication' => $rows['indication'],
			'medicine_side_effect' => $rows['side_effect'],
			'medicine_administration' => $rows['administration'],
