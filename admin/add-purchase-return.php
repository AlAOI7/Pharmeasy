...
$total_purchaseReturnAmount = $medicine->select_all_purches_return_product_info();
$totalAmount2 = 0;
if (is_array($total_purchaseReturnAmount) || is_object($total_purchaseReturnAmount)) {
    foreach ($total_purchaseReturnAmount as $resultTotalAmount2) {
...
$getInvoiceId = $medicine->getReturnInvoiceIdInfo();
...
$invNum = date('y') . $month . $day . "0" . $invoiceID;

//Invoice number generate end
}

$query_result = $medicine->select_return_purComnID_info();
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['btn'])) {
    ...
    $common_id = $medicine->save_purchase_return_main_invoice_info($product_sup_name, $currentDue, $invoice_number, $purchase_date, $totalAmount, $payment_method, $disCnt, $less, $cashPaid, $dues, $predues, $chequeAmount, $bankName, $cheque_num, $cheque_app_date);
    ...
    if ($common_id != '' && $product_sup_name != '') {
        ...
        for ($i = 0; $i < count($pId); $i++) {
            ...
            if ($product_id != '' && $quantity != '' && $cprice != '' && $sprice != '') {
                $insertItem = $medicine->addReturnItem($product_id, $mdate, $edate, $cprice, $sprice, $quantity, $subTotal, $invoice_number, $inStock, $invNum, $product_sup_name, $purchase_date);
            }
            $savecount++;
        }

        $message2 = $medicine->updateProductReturnStatusFlag($invoice_number);
        if ($message2) {
            $message3 = $medicine->updateReturnSupplierDues($currentDue, $product_sup_name);
        }
    }
}
?>
    ...
    <div class="page-breadcrumb">
        <div class="row">
            <div class="col-12 d-flex no-block align-items-center">
                <h3 class="page-title">شراء الدواء / المنتج ارجاع</h3><br>
                <?php
                ...
                <div class="ml-auto text-right">
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item active" aria-current="page">اضافة المشتريات ارجاع</li>
                        </ol>
                    </nav>
                </div>
            </div>
                            ...
                            <div class="form-group">
                                ...
                                <div align="center">
                                    <input type="text" name="search" id="search" class="search form-control"
                                        placeholder="Search Medicine" autocomplete="off" autofocus="autofocus" />
                                ...
                                <table class="table table-bordered table-striped table-hover tbl"
                                    ...
                                    <thead>
                                        <tr style="background-color: #ff9900">
                                            ...
                                            <th class="text-center" style="width: 120px">الاسم</th>
                                            <th class="text-center" style="width: 50px">M.date</th>
                        ...
                        <div class="card-body">
                            <div class="box-body">
                                <div class="row">
                                    <div class="col-md-3">
                                        <div class="form-group">
                                            <label>مورد <span style="color: red">*</span></label>
                                            <select name="product_sup_name" id="sup_id" class="form-control select2"
                                                style="width: 100%;" required>
                                                <option value="">احختار المورد</option>
                                                <?php 
                                    ...
                                    <div class="col-md-2">
                                        <div class="form-group">
                                            ...
                                            <select name="payment_method" class="form-control" id="getFname"
                                                ...
                                                <option value="Pay order">امر الدفع</option>
                                    ...
                                    <div class="col-md-2">
                                        <div class="form-group">
                                            <label>الاستحقاق<span style="color: red">*</span></label>
                                    ...
                                    <div class="col-md-3" id="admDivCheck4" style="display:none;">
                                        <div class="form-group">
                                            <label style="font-size: 14px;">التحقق من القيمة</label><br>
...
$(document).ready(function() {
    ...
    $('.search').on('keypress', function() {
        var li = ""
        var text = ''
        $.getJSON('product2.php?name=' + $(this).val(), function(response) {
            response.forEach(function(value) {
                ...
                li += "<li class='sli' data-id='" + value.id + "' data-stock='" + value
                    ...
                    "' data-sprice='" + value.sale_price + "' id='" + data_field +
                    "'><a href='#'>" + data_field1 + "</a></li>"
            });
            $('.list-group').html(li)
        })
    ...
    function bindTOTemplate(products) {
        ...
        if (products) {
            ...
            products.forEach(function(pro) {
                ...
                product += "<td>"
                product +=
                    "<input type = 'date' style='width: 120px' name='mdate[]' class='form-control' value='' data-id='" +
                    pro.id + "' />"
                ...
                product += "<td>"
                product +=
                    "<input type = 'date' style='width: 160px' name='edate[]' class='form-control' value='' data-id='" +
                    pro.id + "' />"
                product += "</td>"

                product +=
                    "<td><input type = 'text' name='cprice[]' style='width: 100px' class='cprice form-control' value='" +
                    pro.cprice + "' data-id='" + pro.id + "' /></td>"
                product +=
                    "<td><input type = 'text' style='width: 100px' name='sprice[]' class='sprice form-control' value='" +
                    pro.sprice + "' data-id='" + pro.id + "' /></td>"
                product +=
                    "<td><input type = 'text' style='width: 100px' name='quantity[]' class='qty form-control' value='" +
