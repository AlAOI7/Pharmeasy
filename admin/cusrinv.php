...
if (!isset($_GET['invid']) || $_GET['invid'] == NULL) {
    echo "<script>window.location = 'manage-sales-return.php';</script>";
} else {

    $invid = preg_replace('/[^-a-zA-Z0-9_]/', '', $_GET['invid']);
}
$getProductInv = $medicine->getAllReturnProductInvoice($invid);
$getSingleInv = $medicine->getAllReturnSingleInvoice($invid);
...
$pdf->Cell(182, 6, 'Ta-98/C, (Gulshan-Badda Link Raod),', 0, 1, 'R');
...
$pdf->Cell(40, 5, 'Invoice Due', 0, 0);
$pdf->Cell(30, 5, $invDue, 0, 1, 'R');
